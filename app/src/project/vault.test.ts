...
describe('EncryptedStorage vault', () => {
  ...
  it('encrypts ProjectStore durability backups at rest and loads them for recovery', async () => {
    const disk = fakeStorage();
    const vault = await EncryptedStorage.enable(disk, 'pw');
    const store = new ProjectStore(vault);
    // Two differing saves create a backup slot for p1 (holding the first save).
    store.save(project('p1', 'Backup Seal Co (fixture)'));
    store.save(startSession(project('p1', 'Backup Seal Co (fixture)'), 'x').project);
    await vault.flush();

    // The backup on disk is ciphertext, not plaintext.
    const backupRaw = disk.getItem('successor:project-backup:p1');
    expect(backupRaw).not.toBeNull();
    expect(isEnvelope(backupRaw!)).toBe(true);
    expect(backupRaw!).not.toContain('Backup Seal Co');

    // A fresh unlock loads the backup into the working copy so load() can recover from it.
    const reopened = await EncryptedStorage.unlock(disk, 'pw');
    expect(reopened.getItem('successor:project-backup:p1')).not.toBeNull();
    expect(new ProjectStore(reopened).load('p1')).toBeTruthy();
  });

  it('enable() adopts and seals a pre-existing plaintext backup (no leak)', async () => {
    const disk = fakeStorage();
    // Simulate the pre-encryption state: a plaintext project AND its plaintext backup.
    const store = new ProjectStore(disk);
    store.save(project('p1', 'Legacy Co (fixture)'));
    store.save(startSession(project('p1', 'Legacy Co (fixture)'), 'x').project);
    expect(disk.getItem('successor:project-backup:p1')).not.toBeNull();

    const vault = await EncryptedStorage.enable(disk, 'pw');
    await vault.flush();

    // Both the primary and the backup are now sealed - neither leaks plaintext.
    const backupRaw = disk.getItem('successor:project-backup:p1')!;
    expect(isEnvelope(backupRaw)).toBe(true);
    expect(backupRaw).not.toContain('Legacy Co');
  });
